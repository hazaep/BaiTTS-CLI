...
> `docs/BURST_TEST.md` (33/33 llamadas, \~10.9× tiempo real).
...
define solo `speed` no borra el `pitch` inline. Ver OPEN\_QUESTIONS #5.
...
con la misma voz sin explicación. Ver OPEN\_QUESTIONS #6.
...
1. `[[PAUSE:<int>]]` (case-insensitive) → se conserva tal cual.
  - Con decimales o no numérico → **warning + truncar a entero** (no
   ...
   naturalidad). Ver OPEN\_QUESTIONS #8.
...
- En la API, `play` no aplica por default: el audio se entrega por **URL temporal** (`GET /audio/{id}` con TTL \~1 h), no como binario embebido y puede ser reproducido localmente de forma opcional. Ver`ADDITIONS.md` §4 y `OPENAPI.md`.

## Progreso y estimaciones

- CLI: línea de estado `n/N chunks · 12.3 s / ~40 s (ETA)` en `mtts dialog`.
- API: array `chunks` en la respuesta con `(personaje, texto, duración, params)` — la duración medida por chunk alimenta la ETA del dashboard.
