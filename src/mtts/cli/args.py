"""Análisis y validación de argumentos de línea de comandos."""
...
def parse_and_validate_args(argv=None):
    ...
    parser.add_argument('--backend', type=str, required=True, help='Especificar la URL del servidor MultiTTS a llamar (obligatorio)')
    ...
    if args.list:
        allowed_args = ['backend', 'list']
        for arg, value in vars(args).items():
            if arg == 'sub' and value is None:
                continue
            if arg not in allowed_args and value is not None and value is not False and value != '.':
                parser.error("Al usar el parámetro --list, solo se permite proporcionar el parámetro --backend")

    # Verificación para las ramas file o dir
    if args.file or args.dir:
        allowed_args = ['backend', 'file', 'dir', 'out', 'voice', 'volume', 'speed', 'pitch', 'sub', 'blacklist']
        for arg, value in vars(args).items():
            if arg not in allowed_args and value is not None and value is not False and value != '.':
    ...
    if not args.list and not args.file and not args.dir:
        other_args_present = any(
            ...
            for key, val in vars(args).items() if key not in ['backend', 'out']  # 'out' tiene valor por defecto, excluirlo
        )
